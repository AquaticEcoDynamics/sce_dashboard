#' tidetimeseriesUI
#'
#' @description Generates the tide plots UI elements e.g. date inputs, region. Requires @param namespace to individualise elements.
#'
...
tidetimeseriesUI <- function(namespace){
  
  tidesites<- methydro_data %>%
    dplyr::filter(group %in% 'tide') %>% 
    dplyr::select('Site')
  tidevars <- methydro_data %>%
    dplyr::filter(group %in% 'tide') %>% 
    dplyr::select('s_graph_value')
  tidevars <- unique(tidevars$s_graph_value)
    ...
    tags$div(
      wellPanel(
        ...
        fluidRow(
          ...
          column(12,shinycssloaders::withSpinner(plotly::plotlyOutput(outputId="hydroTidetestPlot", height = "500px")))
          )
        )
      )
    )
