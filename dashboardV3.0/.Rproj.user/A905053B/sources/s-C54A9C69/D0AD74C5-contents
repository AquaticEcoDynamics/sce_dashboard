...
awss3Listfiles_sitevar <- function(prefix){
...
s3_list <- grep("DATA.csv",s3_list,value = TRUE)

s3_listnames <- gsub(".*/(.+)_.*", "\\1", s3_list)

s3_listheaders <- paste0(s3_listnames,'_HEADER.csv')


s3_site <- unique(sub("\\_.*", "", s3_listnames))
s3_var <- unique(sub("^[^_]*_", "", s3_listnames))



s3_sitevar <- list("site" = s3_site, "variable" = s3_var)
