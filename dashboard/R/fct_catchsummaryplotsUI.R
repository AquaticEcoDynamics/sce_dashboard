...
library(lubridate)
library(EnvStats)
#Rawdata <- read.csv(file = 'www/DBCA_data_export/DBCA_data_export_2023-07-19_1615.csv')
      ...
      wellPanel(
        fluidRow(
          ...
          column(3,selectInput("select_year_catch",
                               label = "Select year" ,
                               choices = years_catch,  #these choices are given based on region selected (in app_server.R)
                               selected = NULL),
                 style="margin-top: 50px;"),
          column(3,selectInput("select_vars_catch",
                               label = "Select variable" ,
                               choices = vars_catch,  #these choices are given based on region selected (in app_server.R)
                               selected = NULL),
                 style="margin-top: 50px;"),
        ...
        fluidRow(
          ...
          #column(12,shinycssloaders::withSpinner(plotOutput("catchsumPlot"))
          )
        )
      )
    )
