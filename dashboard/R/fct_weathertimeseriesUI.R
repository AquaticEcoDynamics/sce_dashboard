...
weathertimeseriesUI <- function(namespace){
  
  # weathersites <- c("9021","9053")
  # weathervars <- c("Air_Temperature","Wind_Speed")
  dir <- 'data-warehouse/bom/idy'
  weathersites <- awss3Listfiles(prefix = dir)$site
  weathervars <- awss3Listfiles(prefix = dir)$variable
  ...
  return(
    tags$div(
      wellPanel(
        ...
        fluidRow(
          column(12,shinycssloaders::withSpinner(plotly::plotlyOutput(outputId="weatherPlot", height = "330px")))
        )
      )
    )
  )
