...
ANZECC <- import(file = 'www/Estuary_plotting/ANZECC guidelines 2000.xlsx')
...
plotEstSum <- function(plotDataRegion, plotDataYear, plotDataVar){
  ...
  b <- plotDataRegion
      ...
      ANZECC_longer <- ANZECC %>% pivot_longer(cols=colnames(ANZECC)[-1],
                                               names_to='Analytes',
                                               values_to='Values')
      ...
      plot.to.print <- p + stat_summary(data = dffiltered %>% filter(`Financial Year Start` %in% Relevant.Years[! Relevant.Years %in% Reporting.Period.Start]), 
        ...
        xlab('Month') +
        ylab(Plot.Labels[p[["labels"]][["y"]]]) + theme(axis.text.x = element_text(size=8)) + 
